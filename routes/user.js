        ...
        res.send(`
            ...
            <body class="bg-gray-100 flex justify-center items-center min-h-screen p-4">
                <div class="bg-white p-6 md:p-8 rounded-2xl shadow-xl w-full max-w-md">
                    ...
                    <button id="copyBtn" onclick="copyLink('${ssconfLink}')" class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-4 rounded-xl shadow mb-6"><i class="fas fa-copy mr-3"></i> COPY SSCONF LINK</button>
                    ...
                    <form action="/panel/change-server" method="POST"><input type="hidden" name="token" value="${token}"><select name="newServer" class="w-full p-3 border rounded-lg mb-4 outline-none font-semibold">${dropdownOptions}</select><button type="submit" class="w-full bg-gray-800 hover:bg-black text-white font-bold py-3 rounded-lg shadow">Switch Now</button></form>
                </div>
    ...
    try {
        ...
        if (!user.accessKeys || !user.accessKeys[newServer]) {
            try {
                ...
                if (masterResponse.data && masterResponse.data.keys) { user.accessKeys = masterResponse.data.keys; user.markModified('accessKeys'); }
            } catch (err) { console.log("Key Sync Error"); }
        }

        user.currentServer = newServer; await user.save(); await redisClient.del(token); 

        // 🌟 Master Developer ရဲ့ သတ်မှတ်ချက်အတိုင်း ၃ ကြိမ် ပြန်ခေါ်ပေးမည်
        try { 
    ...
    try {
        ...
        if (user && user.accessKeys && user.accessKeys[user.currentServer]) {
            ...
            if (typeof rawConfig === 'object' && rawConfig.server) {
                rawConfig = { server: rawConfig.server, server_port: Number(rawConfig.server_port), password: rawConfig.password, method: rawConfig.method, prefix: rawConfig.prefix || "" };
            }
