    ...
    res.send(`
        ...
        <body class="bg-[#f8fafc] font-sans pb-10">
            ...
            <div class="max-w-6xl mx-auto px-4">
                <div class="mb-8"><h2 class="text-3xl font-black text-slate-800"><i class="fas fa-database text-indigo-500 mr-3"></i> Settings & Backups</h2></div>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div class="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-200 lg:col-span-2">
                        <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-4"><h3 class="text-xl font-black text-slate-800"><i class="fab fa-telegram text-blue-500 mr-2"></i> Telegram Auto-Backup Settings</h3></div>
                        <form action="/admin/save-telegram-settings" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
                            <div><label class="block text-[11px] font-black text-slate-400 mb-2 uppercase tracking-widest">Bot Token</label><input type="text" name="botToken" value="${setting.botToken}" class="w-full border-2 border-slate-200 p-3.5 rounded-2xl outline-none focus:border-blue-500 font-mono text-sm text-slate-700 transition"></div>
                            <div><label class="block text-[11px] font-black text-slate-400 mb-2 uppercase tracking-widest">Admin Chat ID</label><input type="text" name="adminId" value="${setting.adminId}" class="w-full border-2 border-slate-200 p-3.5 rounded-2xl outline-none focus:border-blue-500 font-mono text-sm text-slate-700 transition"></div>
                            <div><label class="block text-[11px] font-black text-slate-400 mb-2 uppercase tracking-widest">Interval (Minutes)</label><input type="number" name="backupIntervalMinutes" value="${setting.backupIntervalMinutes}" min="1" class="w-full border-2 border-slate-200 p-3.5 rounded-2xl outline-none focus:border-blue-500 font-bold text-sm text-slate-700 transition"></div>
                            <div class="md:col-span-3 flex gap-3"><button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-2xl transition shadow-[0_4px_15px_rgba(37,99,235,0.3)] active:scale-[0.98]"><i class="fas fa-save mr-2"></i> Save Telegram Settings</button></div>
                        </form>
                        <div class="border-t border-slate-100 pt-6">
                            <form action="/admin/send-telegram-backup" method="POST" class="m-0" onsubmit="document.getElementById('loadingModal').classList.replace('hidden', 'flex');">
                                <button type="submit" class="w-full bg-slate-800 hover:bg-black text-white font-bold py-3.5 rounded-2xl transition shadow-md active:scale-[0.98]"><i class="fas fa-paper-plane mr-2"></i> Send Full Backup to Telegram Now</button>
                            </form>
                        </div>
                    </div>
