...
const redisClient = require('../config/redis'); // 🌟 Cache ရှင်းရန် ထပ်ထည့်ထားသည်
...
adminApp.get('/', async (req, res) => {
        ...
        <div class="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 border border-slate-100 overflow-hidden flex flex-col">
            <div class="bg-gradient-to-r from-slate-800 to-slate-700 p-4 flex justify-between items-center">
                ...
                <form action="/admin/delete-group" method="POST" onsubmit="return confirm('Group ကို ဖျက်မှာ သေချာပြီလား?');" class="m-0">
                    ...
                    <button type="submit" class="text-white hover:text-red-400 bg-white/10 hover:bg-white/20 p-2 rounded-lg transition"><i class="fas fa-trash-alt"></i></button>
                </form>
            </div>
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100">
                <a href="/admin/group/${encodeURIComponent(g.name)}" class="flex items-center justify-center w-full bg-indigo-600 text-white font-bold py-3 rounded-xl hover:bg-indigo-700 shadow-md transition">Manage Group <i class="fas fa-arrow-right ml-2"></i></a>
            </div>
        </div>`;
    }
    ...
    masters.forEach((m) => {
        ...
        mastersListHtml += `
            <div class="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100 mb-2">
                ...
                <form action="/admin/delete-master" method="POST" onsubmit="return confirm('ဖျက်မှာ သေချာလား?');" class="m-0"><input type="hidden" name="id" value="${m._id}"><button type="submit" class="text-red-500 hover:text-red-700 bg-red-50 p-2 rounded-lg"><i class="fas fa-trash"></i></button></form>
            </div>`;
    });
    ...
    res.send(`
        ...
        <body class="bg-slate-50 font-sans pb-10">
            <nav class="bg-gradient-to-r from-indigo-800 to-indigo-600 text-white shadow-lg p-5 mb-8"><div class="max-w-7xl mx-auto flex items-center justify-between"><div class="font-black text-2xl tracking-tight"><i class="fas fa-shield-alt mr-2 text-indigo-300"></i> PROXY <span class="text-indigo-200 font-light">ADMIN</span></div></div></nav>
            <div class="max-w-7xl mx-auto px-4">
                ...
                <div class="bg-white p-6 md:p-8 rounded-3xl shadow-lg border border-slate-100 mb-10">
                    ...
                    <div class="flex flex-col md:flex-row gap-4 mb-6 pb-6 border-b border-slate-100">
                        <div class="flex-1"><label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Select Master API</label><select id="savedMasterSelector" class="w-full border-2 border-indigo-200 bg-indigo-50 p-3 rounded-xl outline-none focus:border-indigo-500 font-bold text-indigo-800">${masterOptions}</select></div>
                        <div class="flex items-end"><button type="button" onclick="fetchGroupsFromSaved()" id="fetchBtn" class="w-full md:w-auto bg-indigo-600 text-white px-8 py-3.5 rounded-xl font-bold hover:bg-indigo-700 transition"><i class="fas fa-sync-alt mr-2"></i> Fetch Groups</button></div>
                    </div>
                    <form action="/admin/create-group" method="POST" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div><label class="block text-xs font-bold text-slate-500 mb-1 uppercase">1. Select Group</label><select name="masterGroupId" id="masterGroupSelect" required class="w-full border-2 border-slate-200 bg-slate-50 p-3 rounded-xl outline-none focus:border-indigo-500 font-semibold text-slate-700"><option value="" disabled selected>Fetch groups first...</option></select></div>
                        <div><label class="block text-xs font-bold text-slate-500 mb-1 uppercase">2. Local Name</label><input type="text" name="groupName" placeholder="e.g. VIP Gaming" required class="w-full border-2 border-slate-200 p-3 rounded-xl outline-none focus:border-indigo-500 font-bold text-slate-800"></div>
                        <div><label class="block text-xs font-bold text-slate-500 mb-1 uppercase">3. Custom DNS</label><input type="text" name="nsRecord" placeholder="e.g. ns1.yourdomain.com" required class="w-full border-2 border-slate-200 p-3 rounded-xl outline-none focus:border-indigo-500 font-bold text-indigo-700"></div>
            ...
            <script>
                async function fetchGroupsFromSaved() {
                    const selector = document.getElementById('savedMasterSelector').value; const btn = document.getElementById('fetchBtn');
                    if(!selector) return alert("Please select a Master API!");
                    const [ip, key, name] = selector.split('|'); btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Fetching...';
                    try {
                        ...
                        if(data.success) {
                            ...
                            document.getElementById('masterGroupSelect').innerHTML = options; document.getElementById('masterGroupSelect').classList.remove('bg-slate-50');
                            ...
                            btn.innerHTML = '<i class="fas fa-check mr-2"></i> Connected!'; btn.classList.replace('bg-indigo-600', 'bg-green-500'); setTimeout(() => { btn.innerHTML = '<i class="fas fa-sync-alt mr-2"></i> Fetch Groups'; btn.classList.replace('bg-green-500', 'bg-indigo-600'); }, 2000);
...
adminApp.post('/add-master', async (req, res) => {
    try { let { name, ip, apiKey } = req.body; ip = ip.replace(/\/$/, ""); await Master.create({ name, ip, apiKey }); res.redirect('/admin'); } catch (e) { res.status(500).send("Error"); }
});
...
adminApp.post('/api/fetch-master-groups', async (req, res) => {
    try { 
        ...
        const response = await fetchWithRetry(masterIp + '/api/active-groups', null, { headers: { 'x-api-key': masterApiKey }, timeout: 5000 });
    ...
    try {
        ...
        if (groupInfo) { 
            for (const u of users) { 
                try { await fetchWithRetry(groupInfo.masterIp + '/api/internal/delete-user', { username: u.name, token: u.token }, { headers: { 'x-api-key': groupInfo.masterApiKey } }); } catch(e){} 
            } 
        }
...
adminApp.get('/group/:name', async (req, res) => {
    ...
    users.forEach((u) => {
        ...
        <tr class="border-b border-slate-50 hover:bg-indigo-50/30">
            ...
            <td class="p-4 w-48">
                <div class="flex justify-between text-xs mb-1 font-bold"><span class="text-indigo-600">${u.usedGB} GB</span><span class="text-slate-500">${u.totalGB} GB</span></div>
                <div class="w-full bg-slate-200 rounded-full h-1.5"><div class="bg-indigo-500 h-1.5 rounded-full" style="width: ${usagePercent}%"></div></div>
            </td>
            <td class="p-4 text-right flex justify-end gap-2">
                <button id="panelBtn-${u.token}" onclick="copyLink('${webPanelLink}', 'panelBtn-${u.token}', '<i class=\\'fas fa-globe\\'></i>')" class="bg-blue-50 text-blue-600 hover:bg-blue-500 hover:text-white px-3 py-2 rounded-lg text-sm font-bold transition"><i class="fas fa-globe"></i></button>
                <button id="btn-${u.token}" onclick="copyLink('${ssconfLink}', 'btn-${u.token}', '<i class=\\'fas fa-link\\'></i>')" class="bg-slate-100 text-slate-600 hover:bg-green-500 hover:text-white px-3 py-2 rounded-lg text-sm font-bold transition"><i class="fas fa-link"></i></button>
                <form action="/admin/delete-user" method="POST" onsubmit="return confirm('ဖျက်မှာ သေချာပြီလား?');" class="m-0"><input type="hidden" name="token" value="${u.token}"><input type="hidden" name="groupName" value="${u.groupName}"><button type="submit" class="bg-red-50 text-red-500 hover:bg-red-600 hover:text-white px-3 py-2 rounded-lg text-sm font-bold transition"><i class="fas fa-trash"></i></button></form>
            </td>
        </tr>`;
    });
        ...
        <body class="bg-slate-50 font-sans pb-10">
                ...
                <div class="max-w-7xl mx-auto flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <form action="/admin/sync-group-nodes" method="POST" class="m-0" onsubmit="this.querySelector('button').innerHTML='<i class=\\'fas fa-spinner fa-spin mr-2\\'></i> Syncing...';">
                            <input type="hidden" name="groupName" value="${groupName}"><button type="submit" class="bg-indigo-100 text-indigo-700 hover:bg-indigo-600 hover:text-white px-4 py-1.5 rounded-lg text-sm font-bold transition shadow-sm border border-indigo-200 flex items-center"><i class="fas fa-sync-alt mr-2"></i> Sync All Nodes</button>
                        </form>
                    </div>
                </div>
            </nav>
            <div class="max-w-7xl mx-auto px-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div class="md:col-span-2 bg-white rounded-2xl shadow-sm border p-6"><label class="block text-sm font-black text-slate-800 mb-4"><i class="fas fa-user-plus text-green-500 mr-2"></i> Generate New Key</label>
                        <form action="/admin/add-user" method="POST" class="grid grid-cols-1 md:grid-cols-4 gap-3">
                            <input type="hidden" name="groupName" value="${groupName}"><input type="text" name="name" placeholder="User Name" required class="border-2 border-slate-200 p-2.5 rounded-xl outline-none focus:border-indigo-500 font-bold text-sm"><input type="number" name="totalGB" placeholder="Data (GB)" required class="border-2 border-slate-200 p-2.5 rounded-xl outline-none focus:border-indigo-500 font-bold text-sm"><input type="date" name="expireDate" required class="border-2 border-slate-200 p-2.5 rounded-xl outline-none focus:border-indigo-500 font-bold text-sm text-slate-600"><button type="submit" class="bg-indigo-600 text-white rounded-xl py-2.5 font-bold hover:bg-indigo-700 transition text-sm">Create</button>
                        </form>
                    </div>
                    <div class="bg-yellow-50 rounded-2xl shadow-sm border border-yellow-200 p-6"><label class="block text-sm font-black text-yellow-800 mb-2"><i class="fas fa-plug text-yellow-600 mr-2"></i> Update Connection</label>
                        <form action="/admin/update-group-master" method="POST" class="flex flex-col gap-2"><input type="hidden" name="groupName" value="${groupName}"><select name="masterData" required class="w-full border border-yellow-300 bg-white p-2 rounded-lg outline-none font-bold text-xs text-slate-700">${relinkOptions}</select><button type="submit" class="w-full bg-yellow-500 text-white rounded-lg py-2 font-bold hover:bg-yellow-600 transition text-sm">Re-Link Master</button></form>
                    </div>
                </div>
                <div class="bg-white rounded-2xl shadow-sm border overflow-hidden"><table class="w-full text-left"><thead><tr class="bg-slate-100 text-xs uppercase text-slate-500"><th class="p-4">ID</th><th class="p-4">User</th><th class="p-4">Node</th><th class="p-4">Expire</th><th class="p-4">Usage</th><th class="p-4 text-right">Actions</th></tr></thead><tbody>${usersHtml}</tbody></table></div>
            </div>
            <script>function copyLink(link, btnId, origHtml) { var tempInput = document.createElement("input"); tempInput.value = link; document.body.appendChild(tempInput); tempInput.select(); document.execCommand("copy"); document.body.removeChild(tempInput); var btn = document.getElementById(btnId); btn.innerHTML = '<i class="fas fa-check"></i>'; btn.classList.add('bg-green-500', 'text-white'); setTimeout(() => { btn.innerHTML = origHtml; btn.classList.remove('bg-green-500', 'text-white'); }, 2000); }</script>
        </body></html>
    `);
});
...
adminApp.post('/update-group-master', async (req, res) => {
    try {
        ...
        if(masterData) { const [ip, apiKey, name] = masterData.split('|'); await Group.updateOne({ name: groupName }, { masterIp: ip, masterApiKey: apiKey, masterName: name }); }
        res.redirect('/admin/group/' + encodeURIComponent(groupName));
...
adminApp.post('/add-user', async (req, res) => {
    try {
        ...
        const groupInfo = await Group.findOne({ name: groupName });
        if(!groupInfo || !groupInfo.masterIp) return res.status(400).send("Error");
...
adminApp.post('/delete-user', async (req, res) => {
    try {
        const token = req.body.token; const groupInfo = await Group.findOne({ name: req.body.groupName }); const user = await User.findOne({ token: token });
        if (groupInfo && user) {
        ...
        await User.deleteOne({ token: token }); res.redirect('/admin/group/' + encodeURIComponent(req.body.groupName));
    } catch (error) { res.status(500).send("Error"); }
});

// 🌟🌟 NEW: SUPER FORGIVING AUTO-RECEIVE WEBHOOK 🌟🌟
adminApp.post('/api/internal/sync-new-server', async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        const { newServerName, userKeys } = req.body;

        if (!newServerName || !userKeys) return res.status(400).json({ error: "Invalid data" });

        // 1. Check if the API key matches ANY of our saved Master APIs
        ...
        for (const [identifier, newConfig] of Object.entries(userKeys)) {
            ...
            if (user) {
                // 3. Direct DB Injection (Force Overwrite)
...
adminApp.post('/api/internal/sync-user-usage', async (req, res) => {
    try {
        ...
        if (usedGB !== undefined) user.usedGB = Number(usedGB); if (totalGB !== undefined) user.totalGB = Number(totalGB); if (expireDate !== undefined) user.expireDate = expireDate;
