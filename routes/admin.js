...
const crypto = require('crypto'); // Random Token ထုတ်ရန်
    ...
    res.send(`
        ...
        <body class="bg-gray-50 font-sans pb-10">
            <nav class="bg-indigo-700 text-white shadow-md p-4 mb-8">
                <div class="max-w-6xl mx-auto font-bold text-xl"><i class="fas fa-shield-alt mr-2"></i>VPN Admin</div>
            </nav>
            <div class="max-w-6xl mx-auto px-4">
                <div class="bg-white p-5 rounded-xl shadow-sm border mb-8">
                    <form action="/admin/create-group" method="POST" class="flex gap-4">
                        <input type="text" name="groupName" placeholder="Group Name (e.g. VIP Package)" required class="flex-1 border p-3 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none">
                        <button type="submit" class="bg-green-600 text-white px-6 py-3 rounded-lg font-bold shadow-sm">Create Group</button>
                    </form>
                </div>
...
adminApp.get('/group/:name', async (req, res) => {
    ...
    users.forEach((u, index) => {
        ...
        <tr class="border-b hover:bg-gray-50">
            ...
            <td class="p-4 text-sm text-red-500 font-semibold">${u.expireDate}</td>
            ...
            <div class="max-w-6xl mx-auto px-4">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border p-6">
                        ...
                        <hr class="mb-4">
                        <div class="max-h-32 overflow-y-auto">${serversHtml}</div>
                    </div>
                </div>

                <div class="bg-white rounded-xl shadow-sm border overflow-hidden">
                    <div class="p-5 border-b bg-gray-50">
                        <h2 class="text-xl font-bold text-gray-800">Key List</h2>
                    </div>
...
adminApp.post('/add-user', async (req, res) => {
    try {
        ...
        // 🔐 Random Token အလိုလို ထုတ်ပေးခြင်း (Hex Format - 32 characters)
