    ...
    for (const g of groups) {
        ...
        <div class="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 border border-slate-100 overflow-hidden flex flex-col">
            <div class="bg-gradient-to-r from-slate-800 to-slate-700 p-4 flex justify-between items-center">
                <h3 class="text-lg font-bold text-white flex items-center"><i class="fas fa-server text-indigo-400 mr-2"></i> ${g.name} <span class="ml-3 text-xs font-black bg-indigo-500 text-white px-2 py-1 rounded-md shadow-sm border border-indigo-400">${g.masterName || 'API-1'}</span></h3>
            ...
            <div class="p-5 flex-1">
                <div class="flex items-center justify-between mb-3 border-b border-slate-50 pb-3"><span class="text-xs text-slate-400 font-semibold uppercase">Master IP</span><span class="text-xs font-bold text-slate-600 truncate w-32 text-right">${g.masterIp || 'N/A'}</span></div>
                <div class="flex items-center justify-between mb-4"><span class="text-xs text-slate-400 font-semibold uppercase">Custom DNS</span><span class="text-sm font-bold text-indigo-600 truncate max-w-[150px]">${g.nsRecord || 'N/A'}</span></div>
                <div class="flex items-center justify-between"><span class="text-xs text-slate-400 font-semibold uppercase">Active Users</span><span class="text-xl font-black text-slate-800">${userCount}</span></div>
            </div>
...
adminApp.post('/api/fetch-master-groups', async (req, res) => {
    try { let { masterIp, masterApiKey } = req.body; masterIp = masterIp.replace(/\/$/, ""); const response = await axios.get(masterIp + '/api/active-groups', { headers: { 'x-api-key': masterApiKey }, timeout: 5000 });
        if (response.data && response.data.groups) { res.json({ success: true, groups: response.data.groups }); } else { res.json({ success: false, error: "Invalid API" }); }
...
adminApp.post('/create-group', async (req, res) => {
    try {
        ...
        if (groupName && masterGroupId && nsRecord && masterIp && masterApiKey) { let cleanIp = masterIp.replace(/\/$/, ""); await Group.create({ name: groupName, masterGroupId, nsRecord, masterIp: cleanIp, masterApiKey, masterName: masterName || "1" }); }
        res.redirect('/admin');
...
adminApp.post('/delete-group', async (req, res) => {
    try {
        ...
        if (groupInfo) { for (const u of users) { try { await axios.post(groupInfo.masterIp + '/api/user-action', { token: u.token, action: "delete" }, { headers: { 'x-api-key': groupInfo.masterApiKey } }); } catch(e){} } }
...
adminApp.get('/group/:name', async (req, res) => {
    ...
    res.send(`
        <!DOCTYPE html><html><head><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet"></head>
        <body class="bg-slate-50 font-sans pb-10">
            <nav class="bg-white border-b border-slate-200 shadow-sm p-4 mb-6"><div class="max-w-7xl mx-auto flex items-center justify-between"><div class="flex items-center"><a href="/admin" class="text-slate-400 hover:text-indigo-600 mr-4 text-xl"><i class="fas fa-arrow-left"></i></a><span class="font-black text-xl">${groupName}</span><span class="ml-3 text-xs font-bold text-white bg-indigo-500 px-2 py-1 rounded shadow-sm">${groupInfo.masterName || 'API'}</span></div><div class="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200"><i class="fas fa-link text-indigo-400 mr-1"></i> ${groupInfo.masterIp || 'Not Linked'}</div></div></nav>
            <div class="max-w-7xl mx-auto px-4">
